...
Use OWASP threat modelling methodology. Include:
- Threat model summary: at least 6-10 threats categorized by OWASP Top 10 (Broken Access Control, Cryptographic Failures, Injection, Insecure Design, Security Misconfiguration, Vulnerable and Outdated Components, Identification and Authentication Failures, Software and Data Integrity Failures, Security Logging and Monitoring Failures, Server-Side Request Forgery) — present as an HTML table
...
function stripExistingArtifactWatermark(content: string, format: "html" | "markdown"): string {
  ...
  if (format === "html") {
    // Remove any watermark section variants, including numbered headings like "13. Watermark".
    const watermarkHeading = String.raw`<h[1-6][^>]*>\s*(?:\d+\s*[.)-]\s*)?Watermark\s*</h[1-6]>`;

    // Remove canonical generated watermark blocks (heading + generated-by + json pre block).
    cleaned = cleaned.replace(
      new RegExp(
        String.raw`\s*${watermarkHeading}\s*<p[^>]*>\s*Generated by:[\s\S]*?</p>\s*<pre[^>]*>\s*<code[^>]*>[\s\S]*?</code>\s*</pre>\s*`,
        "gi"
      ),
      "\n"
    );

    // Remove any remaining watermark heading sections conservatively until the next heading.
    cleaned = cleaned.replace(
      new RegExp(String.raw`\s*${watermarkHeading}[\s\S]*?(?=<h[1-6]\\b|$)`, "gi"),
      "\n"
    );
  } else {
    // Remove markdown watermark blocks including numbered headings like "## 13. Watermark".
    cleaned = cleaned
      .replace(/\n{0,2}##\s+(?:\d+\s*[.)-]\s*)?Watermark\s*[\s\S]*?(?=\n##\s+|$)/gi, "\n")
      .replace(/\n{0,2}Watermark\s*\n\s*Generated by:[\s\S]*?(?=\n##\s+|$)/gi, "\n");
  }
